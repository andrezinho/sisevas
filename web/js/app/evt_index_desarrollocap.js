...
{	
    ...
    $("#list").on('click', '.printerac', function() {
        var i = $(this).attr("id");
        i = i.split('-');
        id = i[1];        
        //alert(id);
        var ventana=window.open('index.php?controller=desarrollocap&action=printeract&id='+id, 'scrollbars=yes, status=yes,location=yes'); 
        ventana.focus();
