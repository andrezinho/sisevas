...
class TipoPersonal extends Main
{
    ...
    function edit($id ) {
        $stmt = $this->db->prepare("SELECT idtipopersonal,descripcion,estado
			FROM tipo_personal WHERE idtipopersonal = :id");
    ...
    function insert($_P ) {
        $stmt = $this->db->prepare("INSERT INTO tipo_personal (descripcion, estado) VALUES(:p1,:p2)");
	...
	$id=$_P['idtipopersonal'];
        ...
        $stmt->bindParam(':p1', $_P['descripcion'] , PDO::PARAM_STR);
        $stmt->bindParam(':p2', $_P['activo'] , PDO::PARAM_BOOL);
        //$stmt->bindParam(':idtipopersonal',  , PDO::PARAM_INT);
        $p1 = $stmt->execute();
        $p2 = $stmt->errorInfo();
        return array($p1 , $p2[2]);
    }
