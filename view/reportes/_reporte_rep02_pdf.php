...
class PDF extends FPDF {
    ...
    function Header() {
        $maxw = $this->maxw;
        //$this->SetMargins(12,40,20);
        $this->setY(35);
        $this->SetFont('Arial', 'B', 12);
        $this->Ln();
        $this->Image('images/logo.jpg', 160, 27, 33, 13);
        $this->Cell(0, 5, 'REPORTE DE RESULTADOS DE EVALUACION - ANUAL PROMEDIADO', 0, 1, 'L', false);
        //$this->Ln(2);        
        $this->SetFont('Times', '', 9);
        $this->Cell(0, 4, strtoupper(utf8_decode($this->anio . ' | PERFIL: ' . $this->consultorio)), 0, 1, 'L', false);
        $this->SetXY($maxw - 30, 37);
        $fecha = date('d-M-Y');
        $this->Write(10, $fecha);
        $this->Ln(2);
